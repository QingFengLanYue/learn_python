...
class l:
    ...
    def check_name(name):
        s_sql=l.select_sql(name)
        check_res=l.check_exists(s_sql)
        if len(check_res)==0:
            user_passwd=''
            user_key=''
            result_name = 0
    ...
    def check_num(name, passwd, user_name, user_passwd1):
        num = 0
        while num < 3:
            check_log = l.login_check(name, passwd, user_name, user_passwd1)
            # passwd=22
            if check_log == 1:
    ...
    #主函数
    def name_input(self,name,passwd):
        passwd=str(passwd)
        user_name,user_key,user_passwd, result_name = l.check_name(name)


        if result_name == 1 :
            user_passwd1 = l.jiemi(user_key, user_passwd)  # 解密后的密码
            l.check_num(name, passwd, user_name, user_passwd1)
        else :
            write_log=input("是否进行账户的注册(Y/N): ")
            if write_log.upper() == "Y" :
                print("注册的用户名为:{0}".format(name))
                passwd = input(r"请输入密码: ")
                l.passwd_check_num(passwd)
                i_sql = l.insert_sql(name,passwd)
                l.write_name(i_sql)

...
class dealExceldiff:
    ...
    def deal_same(df1,df2):
        ...
        m=[]
        for i in lo:
            f1=df1.loc[df1['id']==i].values.flatten()
            f2=df2.loc[df2['id']==i].values.flatten()
            for j in range(co):
                if f1[j] != f2[j]:
    ...
    def deal_result(file1,file2,file_write):
        df1 = pd.read_excel(file1)
        df2 = pd.read_excel(file2)

        df_same1,df_same2,df_add,df_del=dealExceldiff.add_del(df1,df2)

        deal_df1 = dealExceldiff.deal_same(df_same1, df_same2)
